...
export default function NotFoundGame() {
  ...
  return (
    <section className="page-wrap px-4 pb-10 pt-12 sm:pt-16">
      <div className="island-shell relative overflow-hidden rounded-[2rem] px-5 py-8 sm:px-8 sm:py-10">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3 text-sm">
          <span className="rounded-full border border-[var(--line)] bg-[var(--surface)] px-4 py-2 font-semibold">
            Attempts: {attempts}
          </span>
          <span className="rounded-full border border-[var(--line)] bg-[var(--surface)] px-4 py-2 font-semibold">
            Round: {round + 1}
          ...
          {Array.from({ length: CARD_COUNT }, (_, index) => {
            ...
            return (
              <button
                ...
                className="group aspect-[3/4] rounded-2xl border border-[var(--line)] p-0 transition hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--lagoon)] disabled:cursor-default"
                disabled={!isHidden || foundJoker}
                aria-label={isHidden ? `Pick card ${index + 1}` : `Revealed ${cardFace}`}
              >
                <div
                  className={`flex h-full items-center justify-center rounded-2xl text-3xl shadow-sm transition sm:text-4xl ${
                    isHidden
                      ...
                      : state === 'jackpot'
                        ? 'bg-[linear-gradient(145deg,rgba(45,181,125,0.45),rgba(24,110,77,0.85))] text-white'
