...
export default function NotFoundGame() {
  ...
  const handleCardPick = (index: number) => {
    if (foundJoker || revealed[index] || isShuffling) return
  ...
  const startNextRound = () => {
    ...
    // 2. Wait for animation to finish, then reset board and deal new cards
    setTimeout(() => {
    ...
    }, 400) // matches the duration of deal-out animation
  }

  return (
    <>
      {/* Injecting custom animations directly for portability */}
      <style>{`
        @keyframes card-deal-in {
          0% { opacity: 0; transform: translateY(-80px) scale(0.6) rotate(-15deg); }
          100% { opacity: 1; transform: translateY(0) scale(1) rotate(0); }
        }
        @keyframes card-deal-out {
          0% { opacity: 1; transform: translateY(0) scale(1) rotate(0); }
          100% { opacity: 0; transform: translateY(-80px) scale(0.6) rotate(15deg); }
        }
        .animate-deal-in {
          animation: card-deal-in 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
        }
        .animate-deal-out {
          animation: card-deal-out 0.4s ease-in forwards;
        }
      `}</style>
      ...
      <section className="page-wrap px-4 pb-10 pt-12 sm:pt-16">
        <div className="island-shell relative overflow-hidden rounded-4xl px-5 py-8 sm:px-8 sm:py-10">
          ...
          <p className="mt-3 max-w-2xl text-sm text-(--sea-ink-soft) sm:text-base">
            Oops, this table does not exist. Pick cards to find the hidden
            joker. Every miss reveals a decoy card. Hit the joker and we&apos;ll
            deal you back to safety.
          </p>
              ...
              return (
                <button
                  ...
                  className={`group aspect-3/4 rounded-[10px] bg-transparent p-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--lagoon)] disabled:cursor-default perspective-[1000px] ${
                    isShuffling
                      ? 'animate-deal-out pointer-events-none'
                      : 'animate-deal-in opacity-0'
                  }`}
                  style={{
                    animationDelay: isShuffling ? '0ms' : `${index * 45}ms`,
                  }}
                  disabled={!isHidden || foundJoker || isShuffling}
                  aria-label={
                    isHidden
                      ? `Pick card ${index + 1}`
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <button
              ...
              className="rounded-full border border-[rgba(50,143,151,0.35)] bg-[rgba(79,184,178,0.2)] px-5 py-2.5 text-sm font-semibold text-[var(--lagoon-deep)] transition hover:-translate-y-0.5 hover:bg-[rgba(79,184,178,0.3)] disabled:pointer-events-none disabled:opacity-60 disabled:hover:translate-y-0"
            >
              Shuffle & Play Again
            </button>
            <Link
